...
//
// Two-way (full-duplex) audio+video: the GMF `capture` (camera + mic) is SENT
// to the peer, and the GMF `player` (LCD + speaker) RENDERS the peer's incoming
// audio+video. Echo on the full-duplex audio path is removed by AEC (see aec_).
// Video is optional (audio-only) so audio-only boards (e.g. ESP32-S3-Box-3,
// which has ES7210+ES8311 but no camera/LCD) can run a voice call.
...
class WebrtcCall : public Component {
 ...
 protected:
  ...
  std::string board_type_{"ESP32_P4_DEV_V14"};  // codec_board definition name
  std::string board_config_;                    // inline codec_board definition (optional)
  uint16_t width_{320}, height_{240};
  uint8_t framerate_{15};
  bool video_{true};   // false -> audio-only call (no camera/LCD, e.g. S3-Box-3)
  bool aec_{false};    // acoustic echo cancellation for full-duplex audio
